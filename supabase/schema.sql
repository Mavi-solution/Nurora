...
-- It is the nine migrations in supabase/migrations/ consolidated, with
-- one deliberate difference:
--
--   The user_role enum is created with all four values up front.
--   The migrations add 'support' in a later file because Postgres will
--   not let a new enum value be USED in the transaction that adds it,
--   and the SQL Editor runs this whole script as one transaction. On a
--   fresh database defining it complete is equivalent and safe.
--
-- Safe to run on a new project. NOT idempotent — running it twice will
-- error on objects that already exist.
--
-- Requires the Supabase auth schema (auth.users, auth.uid), which every
-- Supabase project has. It does not create demo accounts.
-- =====================================================================
